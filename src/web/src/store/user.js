...
export const useUserStore = defineStore('user', () => {
  ...
  const setCurrentFamily = (family) => {
    ...
    // 同时存入 selectedFamily（页面读取的 key），补齐 family_id 别名和默认 device_id
    uni.setStorageSync('selectedFamily', {
      ...family,
      family_id: family.id,
      // 华为云固定设备ID（后续可从设备列表动态获取）
      huawei_device_id: '6a2179727f2e6c302f77aaf8_env_monitor_01',
    })
    console.log('[DETECT:F0] selectedFamily 已写入:', JSON.stringify(uni.getStorageSync('selectedFamily')))
  }
